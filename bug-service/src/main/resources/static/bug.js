...
function usendbug() {
	...
	fetch('/bug/' + document.getElementById('id').value, {
		...
		.then(response => refresh(showToast(alert("bug updated!"))));
...
function getProject() {
	fetch('http://localhost:8082/project/')
		...
		.then(json => {
			let radr = JSON.stringify(json)
			radr = JSON.parse(radr)
			let txt2 = "<div class=\"col-sm-4 form-group\"><label><b>Project Ids</b></label><select name=\"idproject\" id=\"idproject\" class=\"form-control\"   >";
			for (let i = 0; i < radr.length; i++) {
